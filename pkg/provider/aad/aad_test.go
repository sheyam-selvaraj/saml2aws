...
func TestAad_processConvergedConditionalAccess_Logging(t *testing.T) {
	...
	// Read test data
	data, err := os.ReadFile("testdata/ConvergedConditionalAccess.html")
	require.NoError(t, err)
	...
	// Test that the function processes without panicking and logs appropriately
	// This should return an error (conditional access blocked) but not panic
	_, err = client.processConvergedConditionalAccess(res, string(data), loginDetails)
	require.Error(t, err)
	require.Contains(t, err.Error(), "conditional access policy blocked authentication")
}

func TestAad_detectConditionalAccessPolicyType_Logging(t *testing.T) {
	idpAccount := &cfg.IDPAccount{
		URL:   "https://login.microsoftonline.com",
		AppID: "test-app-id",
	}
	...
	testCases := []struct {
	...
	}{
		{
			...
			response: &ConditionalAccessResponse{
				SErrorCode: "53003",
				SErrTxt:    "Device compliance required",
				Pgid:       "ConvergedConditionalAccess",
			},
			bodyContent:  "device compliance required",
			expectedType: "device_compliance",
		},
		{
			name: "Location-based policy",
			response: &ConditionalAccessResponse{
				SErrorCode: "53000",
				SErrTxt:    "Access from this location is blocked",
				Pgid:       "ConvergedConditionalAccess",
			},
			bodyContent:  "location restricted",
		...
		},
		{
			...
			response: &ConditionalAccessResponse{
				SErrorCode: "99999",
				SErrTxt:    "Some unknown error",
				Pgid:       "ConvergedConditionalAccess",
			},
			bodyContent:  "unknown error message",
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			// Test that policy type detection works and logs appropriately
			policyType := client.detectConditionalAccessPolicyType(tc.response, tc.bodyContent)
			require.Equal(t, tc.expectedType, policyType)
		})
	}
}