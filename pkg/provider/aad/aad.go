...
AuthProcessor:
	for loopCounter := 0; loopCounter < 50; loopCounter++ { // Add loop protection
		...
		case strings.Contains(resBodyStr, "ConvergedConditionalAccess"):
			...
			res, err = ac.processConvergedConditionalAccess(res, resBodyStr)
...
func (ac *Client) processConvergedConditionalAccess(res *http.Response, srcBodyStr string) (*http.Response, error) {
	var convergedResponse *ConvergedResponse
	var err error
	...
	// Handle conditional access by submitting the form data
	// This typically involves continuing with the authentication flow
	formValues := url.Values{}
	if convergedResponse.SFTName != "" && convergedResponse.SFT != "" {
		formValues.Set(convergedResponse.SFTName, convergedResponse.SFT)
	}
